...
void Application::start()
{
	...
	spriteTypes[10] = new Sprite("../media/Player 1.png");
	spriteTypes[11] = new Sprite("../media/You Are.png");

	startBackground = GameObject(spriteTypes[3], 640, 480, 12.8, 9.6);
	playerTurn = GameObject(spriteTypes[7], 640, 800, 6.4, 0.64);
	playerNumber = GameObject(spriteTypes[1], 850, 200);
	youAre = GameObject(spriteTypes[11], 600, 200, 6.4, 0.64);
...
void Application::playerName(int pID)
{
	if (pID == 0001)
	{
		playerNumber.setSprite(spriteTypes[1]);
	}

	else if (pID == 0000)
	{
		playerNumber.setSprite(spriteTypes[0]);
	}
}
